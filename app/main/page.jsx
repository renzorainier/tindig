...
// ------------------------------------------------------------------------------------------------
const BAD_POSTURE_THRESHOLD_MS = 3000; // 3 seconds
/* ---------- Main Component ---------- */
export default function PoseCamera() {
  ...
  const wasBadPostureRef = useRef(false); // <-- ADD THIS
  ...
  const predict = async () => {
    ...
    // --- NEW: Get real dimensions from the video element ---
    ...
    if (finalLandmarks) {
      ...
      if (calibratingRef.current && metrics) {
        ...
        setTriggeredReasons([
          "Hold a natural, relaxed posture for calibration.",
        ]);
      ...
      if (!calibratingRef.current && metrics) {
        ...
        if (leaningBack) {
          reasons.push(
            "Leaning Back: Too far from the screen. Center yourself."
          );
        }
        ...
        if (reasons.length > 0) {
          // --- NEW: 3-SECOND DELAY LOGIC ---
          if (badPostureStartTimeRef.current === null) {
            // This is the first frame of bad posture. Start the timer.
            badPostureStartTimeRef.current = Date.now();
          }

          // Calculate how long bad posture has been held
          const elapsed = Date.now() - badPostureStartTimeRef.current;

          // Update visual progress bar (clamped between 0 and 1)
          ...
          if (elapsed >= BAD_POSTURE_THRESHOLD_MS) {
            // --- TIMER FINISHED: CONFIRM BAD POSTURE ---
            setStatusMessage("Bad Posture ❌");
            setTriggeredReasons(reasons);

            // Play sound only on the *first frame* this triggers
            if (wasBadPostureRef.current === false) {
              playBadPostureSound();
            }
            wasBadPostureRef.current = true; // Mark as "confirmed bad"
          } else {
            // --- TIMER IS STILL RUNNING ---
            // Show a "holding" message while timer runs
            const secondsLeft = Math.ceil(
              (BAD_POSTURE_THRESHOLD_MS - elapsed) / 1000
            );
            setStatusMessage(`Loosing Good Posture... ${secondsLeft}`);
            setTriggeredReasons(reasons); // Show *why* it's checking
          }
          // --- END OF 3-SECOND DELAY LOGIC ---
        } else {
          // --- GOOD POSTURE: RESET EVERYTHING ---
          setStatusMessage("Good Posture ✅");
          setTriggeredReasons(["Keep it up!"]);

          // Reset timer, visuals, and confirmed state
    ...
    const init = async () => {
      try {
        ...
        if (video && canvas) {
          ...
          // --- NEW: Wait for metadata to get real dimensions ---
          video.onloadedmetadata = () => {
            ...
            // 1. Set the canvas PHYSICAL size to match the video
            canvas.width = actualWidth;
            canvas.height = actualHeight;

            // 2. Adjust the parent container's aspect ratio to prevent CSS squashing
            const container = canvas.parentElement;
            if (container) {
              // Remove the fixed 4:3 aspect ratio class
              container.classList.remove("aspect-[4/3]");
              // Set the new aspect ratio directly via style
              container.style.aspectRatio = `${actualWidth} / ${actualHeight}`;
            }

            // 3. Now start the prediction loop
  ...
  const startCalibration = () => {
    ...
    try {
      ...
      /* Handle potential localStorage access error silently */
    }
    setBaseline(null);
  ...
  const stopAndSaveRecording = async () => {
    ...
    const meanShoulder = meanValue(
      safeSamples,
      (s) => s.metrics.shoulderDiffPx
    );
    const meanHeadOffset = meanValue(safeSamples, (s) => s.metrics.headOffsetX);
    const meanEyeDist = meanValue(
      safeSamples,
      (s) => s.metrics.interEyeDistancePx
    );
  ...
  const playBadPostureSound = () => {
    if (audioAlertRef.current) {
      // Rewind the sound to the start in case it's already playing
      ...
      <div className="w-full max-w-lg bg-white p-6 rounded-xl shadow-xl border border-gray-200">
        ...
        <div
          className={`p-4 rounded-lg font-bold text-center w-full transition-colors duration-500 mb-5 text-lg
                    ${
                      isBadPosture
                        ? "bg-red-100 text-red-700 border-red-300 border-2"
                        : isCalibrating
                        ? "bg-yellow-100 text-yellow-700 border-yellow-300 border-2"
                        : "bg-green-100 text-green-700 border-green-300 border-2"
                    }`}>
          <p className="text-xl">
            {isLoading ? "Loading Pose Model..." : statusMessage}
          </p>
        </div>

        {/* --- ADD THIS VISUAL COUNTDOWN BAR --- */}
      {/* --- MODIFIED: Visual Countdown/Alert Bar --- */}

        {/* State 1: Counting down (yellow bar) - shown when counting > 0 AND not yet "Bad Posture" */}
        {badPostureCountdown > 0 && !isBadPosture && (
          <div className="w-full bg-gray-200 rounded-full h-2.5 mb-4 -mt-2" title="Hold posture...">
            <div
        ...
        {/* State 2: Bad posture confirmed (full red bar) - shown when status is "Bad Posture" */}
        {isBadPosture && (
          <div className="w-full bg-red-100 rounded-full h-2.5 mb-4 -mt-2" title="Bad Posture Detected">
            <div
